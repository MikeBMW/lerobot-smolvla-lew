...
同步 4 处 + 1 行历史:
  ...
  4. tools/gui/studio.py      — changelog 注释前缀 (**只写做了什么 + 根因**)
  5. VERSION.md               — 版本历史表新增一行
...
def main() -> int:
    ...
    resid = len(re.findall(r"\bv%s\b" % re.escape(old), s2 + u + d2)) + len(re.findall(r"\bv%s\b" % re.escape(old), vm2))
    print("  旧版本号残留 (历史行属正常): %d 处" % resid)
    if a.dry:
        print("(--dry: 未写盘)")
        return 0
    for p, txt in ((STUDIO, s2), (UPD, u), (DOCS, d2), (VM, vm2)):
        open(p, "w", encoding="utf-8").write(txt)
    import py_compile
    for p in (STUDIO, UPD, DOCS):
