...
每帧标注 (追加到 state 尾部, 与触觉段并列):
  ...
  [腕部证据 2D]       hand→peg 距离 + peg→hole 距离
  → state 49D + 9D CoT = 58D

数据生成判定 (在 gen_metaworld_data.py 专家循环内):
  phase 判定:
    1 接近:   hand→peg 距离 > 0.08
    2 抓取:   < 0.08 且夹爪未闭合
    3 抬起:   peg z 升高 > 0.02 (已抓起)
    4 插入:   peg→hole < 0.15 (转移后下降)
  contact:  hand→peg < 0.05 (腕部接触)
  sliding:  接触中 peg z 变化 > 0.01 (滑动)
  seated:   peg→hole < 0.05 且 peg z 稳定 (插入到位)
  wrist_evid: [dist(hand,peg), dist(peg,hole)] (腕部局部证据)
