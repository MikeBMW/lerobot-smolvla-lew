#!/usr/bin/env python3
"""🧮 状态空间仿真 完整插拔流程 实测校验 (八阶段 + 光模块独立轨迹)

用法: gui-venv311/bin/python tools/probe_ss_pipeline.py
判定: 八阶段全部走到「完成」+ 光模块真的从台面被抓起(z 上升)+ 光模块头插到 goal (残距 < 4mm)
"""
import os
import sys
...
print(f"  光模块: {np.round(tr['peg'][0], 3)} → {np.round(tr['peg'][-1], 3)}")
print(f"  光模块 z: 初始 {peg_z[0]:.3f} → 最高 {max(peg_z):.3f} (提起 {max(peg_z) - peg_z[0]:.3f}m)")
print(f"  夹住时刻: 帧 {gi} (t={tr['t'][gi]:.2f}s)" if gi is not None else "  ❌ 从未夹住")
print(f"  光模块头终点 {np.round(head_end, 3)}  goal {np.round(HOLE_POS, 3)}  插入残距 {resid * 1000:.1f}mm")
...
print(f"  光模块真被抓起 (z 上升 >8cm): {'✅' if ok_grasp else '❌'}")
print(f"  光模块头插到位 (残距 <20mm): {'✅' if ok_insert else '❌'} ({resid * 1000:.1f}mm)")
