...
FEATURE_LIBRARY = [
    ("A 感知特征", [
        {"id": "A1", "name": "视觉目标检测",
         "desc": "YOLO 2D 检测, 定位工件/孔位",
         "explain": "基于 YOLO 的 2D 目标检测, 实时定位工件/孔位/托盘/料盒等目标, "
                    "输出检测框+类别+置信度, 是感知链的第一环 (YOLO 感知→State 适配)",
    ...
    ("B 决策特征", [
        {"id": "B1", "name": "端到端动作策略",
         ...
         "iface": "OUT", "eng": "ACT/扩散策略 (PretrainedPolicy)",
         "app": "ACT / 端到端"},
        {"id": "B2", "name": "世界模型预测",
         "desc": "预测未来帧/状态, 旁路校正",
    ...
    ("G 接口特征", [
        {"id": "G1", "name": "标准 I/O 接口",
         ...
         "explain": "8 个标准接口 (IN/OUT/CFG/TRAIN/DEPLOY/EVAL/MON/SCHED) 构成模型接入契约, "
                    "第三方模型实现同接口即插即用 (参考 LeRobot PretrainedPolicy)",
