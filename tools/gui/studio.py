...
class PluggingSceneModule(SubModuleWidget):
    ...
    def _build_brick_panel(self):
        ...
        outer = QHBoxLayout(); outer.setSpacing(24)
        ...
        for col_idx, (lvl_name, lvl_yield, lvl_color) in enumerate(levels):
            ...
            for name, l2, l3, l4 in modules:
                ...
                if status == 'active':
                    ...
                    txt = QLabel(f"✦ {name}")
                    txt.setStyleSheet(f"color:{lvl_color}; font-size:10px; font-weight:bold;")
                    state = 'new'
                else:  # keep
                    brick.setStyleSheet(f"background:{C_BG}; border:1px solid {lvl_color}44; border-radius:6px; margin:2px 0;")
                    txt = QLabel(f"  {name}")
                    txt.setStyleSheet(f"color:{C_GRAY}; font-size:9px;")
                    state = 'keep'
