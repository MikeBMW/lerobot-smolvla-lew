...
class SystemSidebar(QFrame):
    ...
    def _build(self):
        ...
        ver = QLabel("Z-MAX v5.5.41")  # 品牌版本小字 (菜单栏右侧有同款, 此处紧凑显示)
...
class StudioMainWindow(QMainWindow):
    def _maybe_warn(self):
        ...
        if not _ok:
            try:
                self.setWindowTitle("XSpace Studio — Z-MAX v5.5.41 [W-01] ⚠️非调试模式")
    ...
    def __init__(self):
        ...
        # v5.5.41: 势场收敛修复 + 逐层对照工具 (小版本迭代) — 管壁高度改 λ=4·k_att·σ² 归一 + 新增近谷锥形吸引项 k_lin=3·k_att·σ → 消除近谷次极小 (收敛 4000 步未达标 → 98~127 步到 <1mm, Φ 严格单调降); 覆盖闸 d_max=500mm + 相位按状态软判; 新增 tools/mem_layer_ablation.py (四臂 AB: off/L2/L23/L234); 势场验证 26/26, 桥 e2e 全关 0 介入 / 开 L2 介入 120/120
