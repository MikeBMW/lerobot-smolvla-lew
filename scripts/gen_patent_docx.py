...
solutions = [
    ('（一）感知层（Sys-2 数据系统）',
     '包括3D深度相机（Gemini 335L）、腕部视觉相机、六维力/扭矩传感器（采样频率1kHz）、'
...
effects = [
    ...
    '力控响应频率1kHz，比传统PLC力控方案（<100Hz）提升100倍，实现微牛级精细力保护，有效避免光模块接口损伤；',
...
doc.add_paragraph(
    ...
    '末端集成电动夹爪（DH-Robotics）和六维力/扭矩传感器（TS-T-15，采样率1kHz）。'
...
l2_steps = [
    ...
    '力控保护：六维力传感器以1kHz频率实时监测插入过程中的Z向力。当Z向力超过预设阈值（通常设为5N）时，系统立即触发急停，机械臂停止运动并保持当前位置。',
...
p.add_run(
    ...
    '（c）执行层，包括采样频率不低于1kHz的关节力控闭环模块，通过gRPC协议与认知层通信，'
    '实现力控自适应插拔和异常自恢复；\n'
...
dep_claims = [
    ...
    ('权利要求7', '根据权利要求1所述的系统，其特征在于，所述六维力/扭矩传感器的采样频率不低于1kHz，力控响应延迟不超过100微秒。'),
