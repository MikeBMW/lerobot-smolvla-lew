...
- **五类数据对象注册表**: `datasets` (扫描 data/, 插销/套环/Orin 各带帧数/eps/state_dim/action_dim/ts) · `curves` (reports/train_curve_*.json → points/tail/ts) · `models` (outputs/train/*/checkpoints 最后一步, policy=目录名 rsplit("_",2)[0]) · `rollouts` (reports/rollout_* 帧数) · `reports` (pdf/mp4)。
...
- **当前训练数据集卡片** (_current_dataset_html): 扫描 config_*.yaml 按 mtime 倒序取 `root: data/...` → 探测 info.json/npz → 显示路径+类型(插销=绿/套环=黄)+帧数。**坑: root 行有 2 空格缩进, 正则必须 `^\s*root:\s*(data/\S+)` (m 标志), 写 `^root:` 匹配不到 → 兜底显示 metaworld_act**。
