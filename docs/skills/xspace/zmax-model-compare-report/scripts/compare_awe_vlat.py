...
def load_models():
    """加载 vla_touch + awe_zflow (从 train_curve_*.json 定位 model.pt)"""
    ...
    for key, mod_name, cls_name in [("vla_touch", "train_vla_touch", "InterpolantPolicy"),
        ...
        cfg = data["config"]
        if key == "awe_zflow":
            # AWE 落盘 cfg 用 d_z 键 (不是 latent_dims); 构造须 d_z1/d_z2/d_z3
...
def run_episode(policy, key, tac_mode, seed=0, steps=120):
    ...
    dists, amps, smooths = [], [], []
    prev_act = None
    # AWE 动作历史必须自回归 (GRU 世界模型需要上下文), 维度 = action_dim
    ...
    for i in range(steps):
        ...
        prev_act = act.copy()
        a4 = np.zeros(4)
        a4[:min(4, len(act))] = act[:min(4, len(act))]
