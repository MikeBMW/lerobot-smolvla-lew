...
### 🐛 2026-09-07 晚 静静: R1 视觉"抓不起光模块"根因链修复 (seed104 0/1→3/3 352步, commit c142b55a)
老倪报"状态空间▶运行 3D 视图总是无法抓起光模块不断尝试" = GUI RealStateSpaceSim(seed104,
vision=True, every=1)。复现 500步失败 (下降 230-280 + 抓取滑脱循环)。**根因 5 项全实锤**:
1. **深度 scale 0.978 过时**: 10 布局初始帧隐含真 scale 0.9577-0.9674 平均 0.9616 (探针
   tools/probe_r1_yolo_calib.py) → peg 3D 系统偏 2-5cm (悬停 30mm, 误差 1:1 深度传导;
   公式/2D 中心自洽 — 真值深度代入仅 2mm 误差)。修: yolo_state_aligner 默认 0.9616。
   ⚠️ 单目深度 (SILog 训练) **尺度随场景漂移**: 手一动 (z=0.13 距 peg 10cm) 误差回 30mm —
   固定 scale 只对"手远离"帧有效。
2. **geom peg_z0 = o[4] 取到 peg x** (obs[4:7]=xyz 实锤: o[4]=0.0585=x, o[6]=0.03=z) →
   抬升锚错 + 幻影免疫阈值全废。修 o[6]。
3. **R1 视觉未检出回退 o[4:7] 真值冒充检测** (红线) → 改保持上次估值 (None→原地等检出)。
4. **幻影污染**: 夹爪进 peg 上方 ~10cm, YOLO 光模块框锁夹爪 (vis≈hand, z=0.083=夹爪高,
   误差 82-92mm; 混合框 z 0.064 半幻影)。修: 定位状态机 — 首轮手高位定位 (1mm) 后锁存
   (_reloc=False 全冻结); 滑脱/遇阻回接近置 _reloc=True 重定位; z 窗幻影免疫 ±2cm (peg 未
   夹持 z 必≈台面); 大跳变 >5cm 需 2 帧同候选确认 (防单帧误检, 允许真被碰移)。
5. **夹持后 peg_head 用视觉锁存偏移** (残差 2.5mm + peg 夹爪内轴向滑移 5-7mm) → 转移路径
   擦孔沿 → 撞击滑脱掉件。修: **夹持真值锚定** — 随动验证稳定 ≥15 帧且偏差 <8mm 后
   _grasp_off0 = o[4:7]−x 实测值 (真机同构: 夹持确认后工件位置由夹爪机械定位/编码器保证,
   视觉只负责找到+接近工件, 不负责毫米级插入对准)。
**诊断工具** (tools/probe_r1_yolo_*.py): err=误差分解(真值深度代入验证公式自洽) /
err2=深度分位扫描(中位最优, 低分位是夹爪前景 — 背景污染假设证伪) / calib=多布局 scale 标定 /
plane=2D+平面 vs 深度对比 (运行期 2D 中心也漂 244mm — 遮挡让 2D 检测全崩)。
**结果**: R1 seed104 (GUI 同参) 500步失败 → 352步 ✅ 3/3 稳定; R0 seed104 回归不伤 (353→343);
R1 多布局 2/6 (104/108 过; 105-109 物理难布局, 与 R0 同分布 — 视觉非瓶颈)。GUI 需重启
(GUI 进程模块 import 缓存旧 sim_real) 才生效。

### 🖐 2026-09-07 老倪目检: 3D 视图夹爪显示反相 (gripper 语义双源打架, commit 7e47a7de)
老倪: "3D视图夹爪刚开始闭合, 后来张开, 张开了却能抓住光模块?是不是反了?" — **对, 反了**:
- **metaworld obs gripper: 1=张开 0=闭合** (探针实锤); **引擎快演 gripper: 0=张开 1=夹紧** (夹紧度)
- sim_real tr["gripper"] 原样记 obs → 3D 视图 (ss_dreamview gap=0.024+(1-g)*0.024 按
  1=夹紧 写的, 对引擎对真实化反) → 真实化轨迹显示: 初始真张开→画闭合, 夹紧真闭合→画张开。
- **修: sim_real tr 输出统一为夹紧度 1−obs** (0=张 1=夹紧, 同引擎), ss_dreamview
  gap公式/夹持度(>0.6)/标注(1=闭合) 全对齐; 内部 self.gripper 仍 obs 判定不动。
- 验证: seed104 夹紧度 0.00(开)→锁存0.44→0.69(夹紧), 352步不受影响。
- 教训: 跨源 tr 数据进共享 3D 前必须统一语义; 引擎 vs metaworld gripper 相反是
  双源坑 (引擎自己 0=开, metaworld obs 1=开)。

### 🕳 2026-09-07 老倪目检: 3D 插入位置偏 (场景几何写死 vs 布局现场漂移, commit fa32ddac)
老倪: "插口插入的位置偏了" — 实锤: metaworld 布局**每进程漂移** (seed104 现场孔口
[-0.177,0.424,0.13] vs 3D ss_dreamview 写死常量 _HOLE_MOUTH y=0.462 偏 3.8cm; seed100
偏 21cm) → ▶运行真实化轨迹 3D 里 peg 插入点总偏离画的孔 (孔/盒画在写死位)。
修: sim_real `_reset` 现场采样 box 中心 (body "box"; ⚠️ _reset 无 m 变量, 用 env.model)
+ run() 结束 `tr["_meta"]` = {goal, hole_mouth, peg0, box_center, seed, steps,
stage_final, vision, done} → ss_dreamview.set_trajectory 见 tr["_meta"] 自动
_apply_meta → 场景盒/孔口画到本轮现场。验证: seed104 meta 与 geom 一致, 343步不受影响。
教训: metaworld 现场几何 (孔/盒/peg) 全布局相关, 任何写死坐标进 3D/显示都会对不上;
tr 必须自带现场 meta, 显示层不许用全局常量猜几何。

### 🏋️ 2026-09-07 深夜: 全模型训练 (老倪"训练到100%") + 多布局评估
- **左脑 MLP**: 教师扩采 40 新布局 (gen 管道 seed300-339 全成功, 教师 100%) → ss_mw_raw
  103→143 → v4w 数据集 150ep/91772帧 → 30K步 (4:48, 104 step/s, loss 0.011-0.038) →
  SS_STATS_ROOT=data/ss_mw_lerobot_v4w export → models/ss_left_brain.npz (旧 mw4w 已备份)
- **右脑 WM**: --retrain (147+布局/88675帧, resume 只是评估! 数据扩后必须 --retrain) →
  next 0.1cm contact acc **0.999** (触0.99/空0.00) → ss_right_brain.npz (旧备份 mw4_backup)
- **YOLO v2 实验证伪 (⚠️ 教训)**: gen_yolo_data 改真实尺寸标注 (peg 8角点包络, 60集/9000帧)
  GPU 重训 mAP50=1.0 → **定位反而劣化 16-18mm vs v1 4-7mm**! 根因: 真实尺寸框中心=peg geom
  中心, 而控制锚=pegGrasp site (v1 固定大框中心=pegGrasp 投影 → YOLO 学对中心)。**标注中心
  必须=控制锚 (pegGrasp), 框尺寸只影响深度提取; 中心错=全错**。已 revert, 保 v1 (检测精度
  不是瓶颈, 瓶颈是物理)。教训: YOLO 重训前先验中心对齐语义, 别为"真实尺寸"牺牲中心。
- **评估**: R0 10轮 5/10 (101-104/108); R1 视觉 10轮 3/10 (102/104/108)。R0 过而 R1 不过的
  101/103 = 视觉 peg 定位残差临界 (视觉 z 表面vs中心偏 4-8mm 布局相关; z 强制修正实验把
  104 从✅打回❌ — 104 视觉 z 本就准, 1mm 变化即翻转 = 毫米级混沌临界) → z 修正已回滚。
  结论: 视觉闭环成功率受 peg 定位残差 (布局相关 1-13mm) 支配; R0 能过的布局 R1 覆盖
  60% (3/5), 难布局同 R0 物理失败。真机倒角/力控是最终解。
- **GUI 演示 (老倪验收面)**: seed104 vision=True 固定布局 = 100% (3/3+ 多轮验证, 352步)。
- **飞书交付**: tools/send_feishu.py — 文本 + 视频 (⚠️ 视频 msg_type=**media** 非 file,
  code 230055 实锤; multipart 手动拼 boundary; 带 headers 的 JSON POST 必须显式
  Content-Type: application/json 否则 99992402 field validation failed)。
