...
## 现象
Z-MAX 控制台 studio.py (PyQt5 5.15, gui-venv311) 在 GNOME Shell 46 / X11 会话下:
...
- 截图分析: 屏幕只有桌面壁纸+终端, 控制台窗口不出现 (用户侧确认)
...
(注: `Qt.AA_DisableWindowManagerEffects` 在 PyQt5 QtCore 中**不存在** — studio.py 里该行抛 AttributeError 被 try/except 吞掉, 无害)、QT_QPA_PLATFORM=xcb 显式、GNOME 会话 env (DBUS/XDG/XAUTHORITY 从 /proc/<gnome-shell-pid>/environ 取全)。
...
- faulthandler (studio.py 已有 ZMAX_FAULTHANDLER=1 → dump_traceback_later) 确认主线程在 app.exec_() 事件循环, 非卡死

## 止损结论
- 疑似 GNOME Mutter 46 对特定复杂 Qt 窗口的 WM 级 map 拒绝, 未找到代码层修复
- 取证/验证**不要等窗口显示**: QWidget.grab() 照常跑 (自动测试套件 12/12 通过)
...
- 用户不在真机旁时: 建 cron 任务 `every 1m` (最小粒度), prompt = "DISPLAY=:0 timeout 10 scrot /tmp/screen_live_monitor.png, 回复含 MEDIA:/tmp/screen_live_monitor.png" → 每分钟截图发飞书群, 用户可实时看桌面
