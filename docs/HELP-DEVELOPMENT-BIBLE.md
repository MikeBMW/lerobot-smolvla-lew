...
> 
> 版本: v1.0.3 | 编制: 智蜂创元(ZFCY)产品管理部 | 生效: 2026-07-06
> 
...
**新特性 (v1.0.3):** 完整的仿真引擎 + 设备树 + 实时监控面板

- **仿真引擎** (`hardware_simulator.py`): 
  - Z700 14-DOF 关节正弦波仿真 + 可调噪声
  - 7路相机彩色条纹测试图案生成
  - 六维力/力矩传感器 (Fx/Fy/Fz/Tx/Ty/Tz)
  - 数字IO模拟 (急停/塔灯/光栅/夹爪/扫码枪)
  - 1ms控制周期，独立线程，不阻塞GUI
  - 故障注入接口 (调试用)
- **模式切换**: 🖥️仿真模拟 | 🔌本地ROS2 | 🤖Orin真机(TCP Bridge)
- **设备树**: QTreeWidget 层级设备列表，点击联动右侧详情面板
- **实时监控**: 
  - 关节状态表 (14-DOF 位置/速度/力矩/温度/电流 100ms刷新)
  - 相机状态 (7路 分辨率/帧率/编码/时间戳)
  - 六维力大数字实时显示
  - IO状态 (急停控制/塔灯/光栅/夹爪)
- **系统架构图**: Z700 ASCII 架构总览 + 设备拓扑表
- **ROS2节点列表**: 8个仿真虚拟节点 + 真机20节点拓扑
- 急停安全配置
...
> 
> 最后更新: 2026-07-06 | 当前版本: v1.0.3 | GitHub: https://github.com/MikeBMW/lerobot-smolvla-lew
