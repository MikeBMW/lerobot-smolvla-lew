...
add_para(doc, "（4）单臂机器人方案：单臂需串行完成取料和插拔，节拍时间长（>15秒/个），无法匹配高速产线需求。")
...
add_para(doc, "（4）如何通过双臂协同将单次插拔节拍压缩至5秒以内。")
...
components = [
    ...
    "（c）控制层：包括双臂协同控制器、力控闭环控制器（带宽>10kHz）、实时安全监控器；",
...
add_para(doc, "（3）双臂异步并行：左臂取料与右臂插拔并行执行，通过中转区解耦，将整体节拍压缩至5秒/个。")

add_heading_styled(doc, "3.2.4 力控闭环系统", level=3)
add_para(doc, "所述力控闭环控制器具有以下特征：")
add_para(doc, "（1）关节力控闭环带宽>10kHz，实现毫秒级力反馈响应；")
...
effects = [
    ...
    ("4. 双臂高效协同", "双臂异步并行将单次插拔节拍压缩至5秒以内，满足高速产线节拍需求。"),
    ("5. 多模态安全冗余", "关节力矩+末端力+触觉三路力信号冗余校验，>10kHz闭环带宽确保人机协作安全。"),
...
add_para(doc, "控制层，包括双臂协同控制器和力控闭环控制器，其中力控闭环带宽大于10kHz；")
...
add_para(doc, "3. 根据权利要求1所述的系统，其特征在于，所述双臂协同控制器采用异步并行控制策略：左臂执行取料工序、右臂执行插拔工序，通过中转区解耦，整体节拍不大于5秒/个。", bold=True)
