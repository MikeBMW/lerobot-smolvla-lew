...
§
画布三级: L4=世界模型(流形JEPA+LEW) / L3=VLM+DiT(smolvla_lew, 模型接管实测90%(v10_r2)≈解析链; 黑帧bug曾致0-15假结论已撤回) / L2=原子技能SK01-08+行左A/B/C算子
§
...
§
网络: Corp-Office/Corp-Guest/Mike热点备选(内网10.163.x, wx2MB/s)。Hermes: CLI≠gateway(system级unit, 判据/proc/pid/cgroup); 飞书99991663=token 2h过期不自刷→重启hermes-gateway.service
§
磁盘红线200G
§
...
§
批量替换代码后必全量语法+行为验证(曾致_render_frame自递归→黑帧假结论); 报结论前验数据有效性(图像std>5)
§
L4档默认=L4Demo真机构链+「🧠流形yaw执行」勾选(流形预测器逐帧发指令φ*); 老倪红线: 真模型/预测器必须默认生效, 面板/日志出现"脚本开环/写死"即不合格(标注免责不算)
§
新功能接入: 完全封装+预留接口+先调通再适配。INTACT(09-11): 论文权重必须paper_runtime(E1权重3-slot=576 vs 根代码four_slot=768不兼容); ckpt需config.json(tar无→hydra compose取model, 顶层须_target_=jepa.JEPA; action_dim=10/action_emb_dim=0); 本机direct sr70%(论文80.22); 缓存~/stable-wm-cache; HF大文件aria2c -x16 -c(.aria2消失=完成, ps grep误判→假损坏); 流水线l4_ab/intact_autopilot.sh