...
§
VSCode调试: F5=🚀全新进程; attach 5678; 断点三坑(09-01): 右键重写launch.json覆盖ZMAX_DEBUG_BREAK→env写死模板+子串过滤; 播放帧数<节点数→n_rounds=max; 置顶弹窗+冻结→not responding点等待; 节点真实执行铁律: 右键源码必真实执行(状态空间9/双脑/YOLO/触觉已接), importlib必sys.modules注册