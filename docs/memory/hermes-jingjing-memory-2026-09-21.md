...
§
AOI10082 v4: /picture=原图, ?kind=crop=拉长960方图(喂YOLO), ?kind=natural=1455x70, /region=区域; 金手指只取焊盘排, 下实心金带/亮边沿不入画布; 伺服tools/aoi_gold_servo.py
§
L2.lissa_insert=里萨如力控插入(产线6N配方,插槽口=沿工具Z退60mm→推进→/lissajous_force_search); 点位insert_pose=产线治具插入位
§
真机视=RealSense帧经 tap 落 ~/zmax_ss_remote/cam_rs.png(YOLO节点ss_yolo_on_real读文件非设备, 看recv.img计数); 力控插入=L2.lissa_insert/lissa_search, 解锁钗出=L2.pull_module(绿环: 退15mm钩环→合爪f30→退120mm)