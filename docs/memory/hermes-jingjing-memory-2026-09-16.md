...
§
崩溃铁律: worker禁QObject→pyqtSignal回主线程; mac GL须主线程; GUI改码必重启; 卡断点'无响应'→mutter check-alive-timeout=0
§
...
§
跨会话: git log+session_search查证; 启训前nvidia-smi协商GPU; 报进度须说清'实际在跑什么'; ssh上pkill -f模式串会自杀→锚定^python3或按PID
§
Hermes: CLI≠gateway; 飞书99991663=token过期→重启hermes-gateway
§
磁盘红线300G
§
评估铁律: 布局每进程漂移禁写死几何; 单次不可靠必多重复+同口径; 每臂独立进程+同解释器; 肌肉记忆A/B须SS_MUSCLE_PATH冷隔离; 预算≥引擎cap
§
...
§
守卫: TOL=0.15→100%+参与21-34%; 53D完备未破参与上限24-27%, >30%掉分/全模型崩; insert_depth 6mm太松→0.002; tr[peg]=速度(位置用peg_head())
§
删大文件先验依赖
§
...
§
断点: 动作头loss行351仅画布「训练」节点进; L3真跑须SS_L3_DEV=cpu; 前馈MLP仅SS_USE_MLP=1进(parallel.py:143), 否则372行覆盖forward→n_mlp恒0
§
口径铁律: 运行时须=训练(图像/255+ImageNet, stats同源, 零回退勿两端零初始→通道死); 多轮一致差先查口径勿加训练量
§
L4'接近'真因(v5.6.3): 直驱反向cos−0.14→修SS_DIRECT_COS_MIN=0.9+否决步交回引擎; L4预算4000步; INTACT CPU≈0.11s/步
§
ThinkBook16p G5: HDMI走dGPU→modeset=1+xorg-nvidia-580(包hold); ~/bin/dual_screen_setup.sh
§
Orin ROS=domain0; tcp_pose 50Hz真值=z7'手/头'项, 目标/光模块点须现场示教(ss_geom_calib可在4060容器跑); force_torque同名双类型(WrenchStamped报invalid allocator→只订JointState); gripper_pos/motion.active_states空闲无发布者; 控制台=systemd --user zmax-studio(杀旧与启动分两次)
§
