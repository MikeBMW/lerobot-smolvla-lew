...
NODE_TREE = {
    ...
    "sssensor": {
        "name": "📡 融合定位", "fb": "底空间·状态观测",
        "funcs": [
            {"fid": "FNsens01", "name": "多源观测融合", "desc": "融合定位模块把视觉系统给出的 39 维图像平面观测与触觉系统给出的 4 维触觉观测拼接融合, 合成一个 43 维的统一观测向量, 供统一状态编码环节使用。",
             "tests": [
            ...
            {"fid": "FNsens02", "name": "时序上下文拼接", "desc": "融合定位模块把当前时刻的观测帧与上一时刻的历史观测帧拼接起来, 构成同时包含时间先后与空间对应关系的上下文结构, 再整体交给状态估计环节使用。",
             "tests": [
    ...
    "ss2d3d": {
        ...
        "funcs": [
            ...
            {"fid": "FN2d06", "name": "视觉伺服引导闭环", "desc": "解算出的三维位姿被直接用作引导信号: 由目标点与末端位姿生成引导向量, 驱动伺服收敛, 使末端在接近、对位、下降与转移各段逐步贴近目标, 横向偏差与法向偏离随过程收敛到毫米以内。",  # noqa: E501
             "tests": [
                 ("引导向量逐帧在位", "auto", "t_F_B12", ""),
                 ("横向偏差收敛 (后1/3 ≤ 前1/3×0.5)", "auto", "t_F_B12", ""),
                 ("末端法向偏离 <1mm", "auto", "t_F_B12", ""),
                 ("静态向量方向口径不适用", "manual", None, "目标向量不含阶段子目标 → 方向一致率仅 ~0.21, 判据用偏差收敛 (已在用例内说明)"),  # noqa: E501
             ]},
        ],
    },
    "sstactile": {
        ...
        "funcs": [
            ...
            {"fid": "FNtac06", "name": "触觉反馈闭环", "desc": "触觉与力觉信号构成闭环: 触觉向量与夹持状态保持互补一致, 力信号映射为接触概率并与力值强相关, 接触段接触概率稳定在 0.99 以上; 同时末端力被安全边界约束在阈值内, 力保护在该闭环内真实生效。",  # noqa: E501
             "tests": [
                 ("触觉通道↔状态互补一致 (≥0.99)", "auto", "t_F_B13", ""),
                 ("力→接触概率联动 corr(contact_p,|F|) ≥0.8", "auto", "t_F_B13", ""),
                 ("插入段接触概率中位 ≥0.9", "auto", "t_F_B13", ""),
                 ("力保护: 接触力上界 ≤1N", "auto", "t_F_B13", ""),
                 ("前向/侧向力通道", "manual", None, "引擎通道 2/3 恒 0 = 真机触觉缺口 (已如实标注, 未冒充)"),
             ]},
        ],
    },
    "ssaoi": {
