    ...
    "L2": {
        ...
        "funcs": [
            ...
            {"fid": "L2-A11", "name": "物理执行", "desc": "执行器→物理世界闭环 (metaworld 真实物理)",
             "groups": ["act", "world"]},
            {"fid": "L2-A12", "name": "3D 视觉引导", "desc": "视觉引导闭环 (视觉伺服): 2D→3D 解算位姿 → 引导向量 → "
             "伺服收敛。实测 (真物理引擎 seed104 400 步): 横向偏差前1/3中位 140.8mm → 后1/3 38.5mm (**0.27×**), "
             "末端法向偏离 148.3mm → **0.3mm** (mani_dperp 首末)。位姿来源: R0 几何真值 / R1 真帧 YOLO detect_3d "
             "(F-C01/C02 另测)。**如实标注**: \"静态目标向量 vs 逐帧速度\"方向一致率仅 12.3% → 该口径不合适, "
             "判据用**偏差收敛 + 法向偏离归零** (用例 F-B12)。",
             "groups": ["2d3d", "sobs", "yolo"]},
            {"fid": "L2-A13", "name": "触觉反馈闭环", "desc": "力/触觉 → 接触概率 → 力保护闭环。实测: "
             "corr(contact_p, |force|) = **0.974** (力→接触概率真实联动) · 插入段 contact_p 中位 **0.9918** · "
             "力上界 **0.672N** (≤1.0 力保护未越界); 触觉通道 obs[39:43] = [原始开度, grasped, 0, 0] 与状态"
